        ...
        HStack(spacing: 12) {
            if let first = expense.remark?.first{
                Text(String(first).capitalized)
                    ...
                    .background(Color(expense.color ?? "Yellow"),in: Circle())
            ...
            VStack(alignment: .trailing, spacing: 7) {
                let currenyString = expenseViewModel.convertNumberToPrice(value: CGFloat(expense.type == "Expenses" ? -expense.amount : expense.amount))
                Text(currenyString)
                    .font(.callout)
                    .opacity(0.7)
